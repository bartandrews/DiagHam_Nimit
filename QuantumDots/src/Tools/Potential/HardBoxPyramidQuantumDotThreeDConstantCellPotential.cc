...
void HardBoxPyramidQuantumDotThreeDConstantCellPotential::ConstructPotential(double noInNProbability, double withInNProbability, double downField, double wettingField, double dotField, double upField, double offset, double cellSizeZ, bool scratch, char* fileName)
{
                ...
                if (scratch)
                {
                    if (this->Neighborhood(i, j, k))
                        if ((double(rand())/RAND_MAX) < withInNProbability)
                            ...
                            Alloy[k][j][i] = false;
                    else if ((double(rand())/RAND_MAX) < noInNProbability)
                        Alloy[k][j][i] = true;
                    else
                        Alloy[k][j][i] = false;
                }
    ...
    tmp1 = (wettingField * this->WettingWidth + dotField) * cellSizeZ;
    tmp2 = tmp1 - offset;
    double tmp3 = wettingField * this->WettingWidth * cellSizeZ;
    for (int j = 0; j < NumberY; ++j)
        for (int i = 0; i < NumberX; ++i)
        {
            // in the dot
    ...
    {
        tmp1 = (wettingField * this->WettingWidth + dotField * (k + 1 - this->Under - this->WettingWidth)) * cellSizeZ;
        tmp2 = tmp1 - offset;
        for (int j = 0; j < NumberY; ++j)
            for (int i = 0; i < NumberX; ++i)
            {
