            ...
            for (int i = this->NbrEigenvalue - 1; i >= 0; --i)
            {
                        ...
                        (this->EigenvaluePrecision * Norm(this->ComplexDiagonalizedMatrix[i])))
                {
                    if (Norm(ComplexDiagonalizedMatrix[i]) > MACHINE_PRECISION)
                        return false;
                    else if (Norm(this->ComplexPreviousWantedEigenvalues[i]) > MACHINE_PRECISION)
                        return false;
                }
            }
            return true;
        }
        else if (Norm(this->ComplexDiagonalizedMatrix[this->NbrEigenvalue - 1] - this->ComplexPreviousWantedEigenvalues[this->NbrEigenvalue - 1]) <
