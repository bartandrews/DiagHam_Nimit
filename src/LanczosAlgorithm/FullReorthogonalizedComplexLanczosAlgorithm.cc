            ...
            for (int i = this->NbrEigenvalue - 1; i >= 0; --i)
            {
                        ...
                        (this->EigenvaluePrecision * fabs(this->DiagonalizedMatrix.DiagonalElement(i))))
                {
                    if (fabs(this->DiagonalizedMatrix.DiagonalElement(i))>3*MACHINE_PRECISION)
                        return false;
                    else if (fabs(this->PreviousWantedEigenvalues[i])>3*MACHINE_PRECISION)
                        return false;
                }
            }
            return true;
        }
        else if (fabs(this->DiagonalizedMatrix.DiagonalElement(this->NbrEigenvalue - 1) - this->PreviousLastWantedEigenvalue) <
