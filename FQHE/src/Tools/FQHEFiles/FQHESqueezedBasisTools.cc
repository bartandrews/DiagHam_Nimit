...
int CountExtendedRootStates(int lzMax, char **referenceState)
{
    ...
    for (int j = 0; j <= lzMax; ++j)
    {
        ...
        else if ((referenceState[j][0] == 'u') || (referenceState[j][0] == 'U'))
            ++NbrUp;
        else if ((referenceState[j][0] == 'd') || (referenceState[j][0] == 'D'))
            ++NbrDown;
        else if ((referenceState[j][0] == 's') || (referenceState[j][0] == 'S'))
            ++NbrSinglet;
    }
    if (NbrSinglet&1)
    {
...
void GenerateExtendedRootStates(int lzMax, int& pos, int **&referenceStates, char **extReferenceState)
{
    ...
    for (int j = 0; j <= lzMax; ++j)
    {
        if (extReferenceState[j][0] == '0')
            Background[j] = 0;
        else if ((extReferenceState[j][0] == 'x') || (extReferenceState[j][0] == 'X'))
            Background[j] = 3;
        else if ((extReferenceState[j][0] == 'u') || (extReferenceState[j][0] == 'U'))
            Background[j] = 2;
        else if ((extReferenceState[j][0] == 'd') || (extReferenceState[j][0] == 'D'))
            Background[j] = 1;
        else if ((extReferenceState[j][0] == 's') || (extReferenceState[j][0] == 'S'))
        {
    ...
    for (int i = 0; i < nbrReferenceStates; ++i)
    {
        ...
        else
        {
            ...
            for (int j = 0; j <= lzMax; ++j)
            {
                ...
                else if ((TmpReferenceStates[i][j][0] == 'x') || (TmpReferenceStates[i][j][0] == 'X'))
                    referenceStates[pos][j] = 3;
                else if ((TmpReferenceStates[i][j][0] == 'u') || (TmpReferenceStates[i][j][0] == 'U'))
                    referenceStates[pos][j] = 2;
                else if ((TmpReferenceStates[i][j][0] == 'd') || (TmpReferenceStates[i][j][0] == 'D'))
