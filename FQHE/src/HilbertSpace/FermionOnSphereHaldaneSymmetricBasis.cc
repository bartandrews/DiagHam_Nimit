...
int FermionOnSphereHaldaneSymmetricBasis::FindStateIndex(unsigned long stateDescription, int lzmax)
{
    ...
    if (CurrentState == stateDescription)
        return PosMid;
    else if (((this->StateDescription[PosMin] & FERMION_SPHERE_SYMMETRIC_MASK) != stateDescription) && ((this->StateDescription[PosMax] & FERMION_SPHERE_SYMMETRIC_MASK) != stateDescription))
        return this->HilbertSpaceDimension;
    else
        return PosMin;
}
